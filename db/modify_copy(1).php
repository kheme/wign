...
for ($i = 0; $i < $columns-1; $i++) {echo "<tr><td width='50%' align='right'>".mysql_field_name($fields, $i) . "</td><td width='50%'><a onclick='if (!confirm(\"Are you sure?\")) {return false}' href='?drop=$name&delete=".mysql_field_name($fields, $i)."'>Delete</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a onclick='if (confirm(\"Are You Sure?\")) {location=\"?new=\"+prompt(\"New Name\",\"".mysql_field_name($fields, $i)."\")+\"&tab=$name&rename=".mysql_field_name($fields, $i)."\"}' href='#'>Rename</a></td></tr>";}
echo "</table>";
...
$id=ge("delete");
$drop=ge("drop");
if (mysql_query("alter table `$drop` drop `$delete`")) {echo "<p align='center'>Done!<br>&nbsp;<br><a href='?table=table&submit=Proceed'>OK</a></p>";}
}
else if (isset($_GET['rename'])) {
$id=ge("rename");
$table=ge("tab");
$new=ge("new");
if (mysql_query("alter table `$table` change `$id` `$new` text")) {echo "<p align='center'>Done!<br>&nbsp;<br><a href='?table=table&submit=Proceed'>OK</a></p>";}

}
else if (isset($_GET['tname'])) {
$submit=$_GET;
